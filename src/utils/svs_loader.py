...
class SVSLoader:
    ...
    def get_0_indexed_svs_for_all_cases(self):
        ...
        case_paths = create_full_paths_to_directories_in_directory_path(self.TCGA_download_directory_path)
        for case_path in case_paths:
            image_paths_of_case = create_full_paths_to_files_in_directory_path(case_path)
    ...
    def get_0_indexed_svs_dict_indexed_by_CID(self):
        ...
        for zero_indexed_svs in self.zero_indexed_svs_images_for_all_cases:
            svs_case_id = zero_indexed_svs.split('/')[-1][:-4]
